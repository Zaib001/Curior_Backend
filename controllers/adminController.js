...
exports.getAllParcels = async (req, res) => {
  try {
    const parcels = await Parcel.find().populate('sender', 'email').populate('driverId', 'name');
...
// PATCH /admin/assign-driver
exports.assignDriver = async (req, res) => {
  try {
    const { parcelIds, driverId } = req.body;

    if (!parcelIds || !Array.isArray(parcelIds) || !driverId) {
      return res.status(400).json({ message: 'parcelIds and driverId are required' });
    }
    ...
    res.status(200).json({ message: 'Driver assigned to parcels', result });
  } catch (err) {
